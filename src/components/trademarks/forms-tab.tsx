...
import { Download, FileText, Settings, Eye, EyeOff } from 'lucide-react';
import { toast } from 'sonner';
import { fillEipaForm, type FormData } from '@/lib/pdf/actions';
import { fillEipaFormClient } from '@/lib/pdf/client-actions';
...
export function FormsTab({ trademark }: FormsTabProps) {
  ...
  useEffect(() => {
    ...
    debounceTimerRef.current = setTimeout(async () => {
      if (!formData.applicant_name) {
        // Don't generate if no applicant name
      ...
      try {
        const pdfBytes = await fillEipaFormClient(formData);
  ...
  }, [formData, showPreview]); // Only depend on formData and showPreview
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      // const base64 = btoa(binary); // Unused, removing to fix lint warning
      ...
      <div className={`grid gap-6 ${showPreview ? 'lg:grid-cols-2' : 'grid-cols-1'}`}>
        ...
        {showPreview && (
          <div className="lg:sticky lg:top-4 lg:self-start">
            <Card className="glass-card smooth-corners h-[calc(100vh-8rem)]">
